# %% [markdown]
#
# 45 points in initial test
#
# This is an implementation demo for the ROI Algorithm. It prioritizes nodes based on their value as a
#    function of points, cost to reach, and strategic importance (centrality). These metrics were hardcoded
#    as weights to find the total "score" for each node. The algorithm then applies the "next best path" logic
#    to find valid paths to the highest scoring nodes while avoiding blacklisted nodes. It runs a loop of the
#    analysis and pathfinding to continuously select new targets as the game state changes.
#
...
if status.get('starting_node'):
    ...
    result = client.select_starting_node("Sydney, Australia")
    print(result)
...
def analyze_quantum_targets(G, start_node):
    ...
    for node_id in G.nodes():
        ...
        # INCREASED AGGRESSION: We square the points so 50 pts is worth 
        # 100x more than 5 pts, instead of just 10x.
        # This forces the General to favor 'Fortress' nodes.
        roi = (points ** 2) / (cost + 1)
...
# %%
# --- MASTER STRATEGY: HARD-SYNC FRONTIER ---
local_blacklist = set()
locally_captured = set() # Manual memory to prevent double-spending

while True:
    # 1. THE REFRESH: Get latest status and sync memory
    status = client.get_status()
    server_owned = set(status.get('owned_nodes', []))
    
    # search_context is the 'Forbidden Zone'
    search_context = server_owned.union(locally_captured).union(local_blacklist)

    possible_moves = []
    for owned_node in server_owned:
        targets = analyze_quantum_targets(G, owned_node)
        for t in targets:
            # ROI Brain is forbidden from picking anything in the Forbidden Zone
            if t['node'] not in search_context:
                if G.has_edge(owned_node, t['node']):
    ...
    if not possible_moves:
        print("🚩 Mission Complete: No valid targets left.")
        break
    ...
    # 3. EXECUTION: Re-inserting the missing runners
    success = False
    if edge_info:
        ...
        # Explicitly call your deterministic circuits
        ...
        if res and res.get("ok") and res.get("data", {}).get("success"):
            ...
            locally_captured.add(next_hop) # IMMEDIATELY block this node
            success = True
    
    if not success:
        print(f"🚫 BLOCK: Adding {next_hop} to persistent blacklist.")
        local_blacklist.add(next_hop) # Deterministic failure means a permanent wall
