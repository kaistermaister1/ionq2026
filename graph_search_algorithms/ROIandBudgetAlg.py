# %% [markdown]
#
# 61 points in initial test
#
# This is an implementation of the ROI and Budget-Aware Algorithm. It extends the basic ROI strategy by 
# including budget information into the calculation for each path. If a path's total cost exceeds the budget,
# the path is discarded. This ensures that the algorithm only considers feasible paths that can be executed 
# within the player's current budget constraints, which shift throughout the game.
#
...
if client and client.api_token:
    ...
    PLAYER_ID = "willwantsROIv3andbudgetblock"
...
if status.get('starting_node'):
    ...
    result = client.select_starting_node("Haifa, Israel")
    print(result)
...
# %%
# --- MASTER STRATEGY: BUDGET-AWARE FRONTIER ---
local_blacklist = set()
locally_captured = set()

while True:
    status = client.get_status()
    current_budget = status.get('budget', 0) # Track our remaining resource
    ...
    for owned_node in server_owned:
        ...
        for t in targets:
            if t['node'] not in search_context and G.has_edge(owned_node, t['node']):
                # NEW: BUDGET GUARDRAIL
                # We identify the edge difficulty and its fixed Bell pair cost
                edge_diff = G[owned_node][t['node']].get('difficulty_rating', 1)
                
                # Determine cost: N=2 for D2, N=3 for D3/D4
                ...
                if current_budget >= required_n:
                    ...
                    print(f"⚠️ Budget too low for {t['node']} (Needs {required_n}, have {current_budget})")
