# %% [markdown]
#
# 31 points in initial test
#
# This was our second implementation of a graph search algorithm using our attack circuits. It was the first
# of ours to score a notable number of points (31).
#
...
if client and client.api_token:
    ...
    PLAYER_ID = "willhatesclaudemorenow"
...
if status.get('starting_node'):
    ...
    result = client.select_starting_node("Amsterdam, Netherlands")
    print(result)
...
# %%
# --- MASTER STRATEGY: GLOBAL FRONTIER RESET ---
local_blacklist = set()

while True:
    # 1. GROUND TRUTH: Get the latest owned nodes from the server
    status = client.get_status()
    server_owned = set(status.get('owned_nodes', []))
    
    # 2. FRONTIER SCAN: Find all possible 'next steps' from our territory
    possible_claims = []
    
    # We look at every node we own as a potential starting point
    for owned_node in server_owned:
        # Check every neighbor of this owned node in the graph G
        for neighbor in G.neighbors(owned_node):
            # If the neighbor is unowned and NOT blacklisted, it's a frontier target
            if neighbor not in server_owned and neighbor not in local_blacklist:
                # Use your strategic weights (Difficulty/Bonus) to find the cost
                edge_weight = G[owned_node][neighbor].get('weight', 1.0)
    ...
    # 3. SELECTION: Pick the absolute cheapest frontier edge in the entire empire
    if not possible_claims:
        print("🚩 No reachable unowned nodes found. Mission complete.")
        break

    # Sort by cost (lowest first)
    possible_claims.sort(key=lambda x: x['cost'])
    best_move = possible_claims[0]
    
    source_city = best_move['source']
    next_hop = best_move['target']

    # 4. EXECUTION: Single-step capture
    success = False
    print(f"\n[FRONTIER] Best move: {source_city} -> {next_hop} (Cost: {best_move['cost']:.2f})")
    
    # We use 3 attempts because D4s are deterministic; if it fails, the wall is real
    for attempt in range(1, 4):
        # Identify Difficulty & Deploy Circuit
        claimable = client.get_claimable_edges()
        ...
        if edge_info:
            ...
            if res and res.get("ok") and res.get("data", {}).get("success"):
                print(f"🎉 Captured {next_hop}! Resetting global strategy.")
                success = True
                break
    
    # 5. PIVOT: If the 'cheapest' edge fails, blacklist it so we don't ping-pong
    if not success:
        print(f"🚫 BLOCK: {next_hop} is unreachable from {source_city}. Blacklisting node.")
        local_blacklist.add(next_hop)
    
    # 6. RESET: The loop restarts, fetches fresh status, and finds the next 'new' cheapest edge
    time.sleep(0.5)
